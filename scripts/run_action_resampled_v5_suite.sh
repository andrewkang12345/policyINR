#!/usr/bin/env bash
# Minari-ID + synthetic-OOD action-resampled v5 MuJoCo suite with 10x longer episodes and history.
set -euo pipefail
cd "$(dirname "$0")/.."
...
N_GPUS="${N_GPUS:-3}"
SEEDS="${SEEDS:-0,1}"
SUITE_ROOT="${SUITE_ROOT:-outputs/mujoco/suites/action_resampled_v4_hopper10x}"
BUILD_ROOT="${BUILD_ROOT:-${SUITE_ROOT}/build}"
RUNS_ROOT="${RUNS_ROOT:-${SUITE_ROOT}/runs}"
CUSTOM_ROOT="${CUSTOM_ROOT:-${RUNS_ROOT}/custom_mujoco_action_resampled_v5}"
...
CUSTOM_DATASETS="${CUSTOM_DATASETS:-custom_mujoco_action_resampled_v5_hopper,custom_mujoco_action_resampled_v5_halfcheetah,custom_mujoco_action_resampled_v5_walker2d,custom_mujoco_action_resampled_v5_ant,custom_mujoco_action_resampled_v5_humanoid}"
...
echo
echo "== action-resampled v5 suite aggregate =="
python -m eval.summary "${RUNS_ROOT}" --out "${SUITE_ROOT}/aggregate.csv" --md "${SUITE_ROOT}/aggregate.md"
echo "Action-resampled v5 suite complete. Results in ${SUITE_ROOT}/"
