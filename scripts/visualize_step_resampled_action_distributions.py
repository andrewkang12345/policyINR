"""Visualize state-resampled MuJoCo action distributions and write a short note.

Outputs land under:
  outputs/suites/state_resampled_*/analysis/action_distributions/
"""
...
def _write_note(results: dict, out_path: Path):
    lines = []
    lines.append("Step-resampled MuJoCo action-distribution analysis")
    lines.append("")
    ...
    lines.append("- Each policy plot shows the action cloud split by ID and OOD states for that policy.")
    lines.append("")
    lines.append("Main conclusion")
    lines.append("- Actions remain substantially more policy-separable than states, which explains why policy-ID probe accuracy stayed high after state matching.")
    lines.append("")
    ...
    lines.append("- The step-resampled construction aligned the observation marginals much better than the action marginals.")
    lines.append("- Since the encoder sees state-action history, it can still recover policy identity from consistent control preferences.")
    lines.append("- The tasks with the highest residual action separability, especially Ant and Humanoid, are the ones where overall probe accuracy stayed closest to 1.0.")
    out_path.write_text("\n".join(lines) + "\n")
...
def main():
    ...
    for env_name, dataset_id in env_ids.items():
        ...
        actions_by_policy_split = _load_env_data(dataset_id)
