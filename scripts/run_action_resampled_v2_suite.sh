#!/usr/bin/env bash
# Shared-state-bag v2 action-shift MuJoCo suite.
set -euo pipefail
cd "$(dirname "$0")/.."
...
N_GPUS="${N_GPUS:-4}"
SEEDS="${SEEDS:-0,1}"
SUITE_ROOT="${SUITE_ROOT:-outputs/mujoco/suites/action_resampled_v2}"
BUILD_ROOT="${BUILD_ROOT:-${SUITE_ROOT}/build}"
RUNS_ROOT="${RUNS_ROOT:-${SUITE_ROOT}/runs}"
CUSTOM_ROOT="${CUSTOM_ROOT:-${RUNS_ROOT}/custom_mujoco_action_resampled_v2}"
...
CUSTOM_DATASETS="${CUSTOM_DATASETS:-custom_mujoco_action_resampled_v2_hopper,custom_mujoco_action_resampled_v2_halfcheetah,custom_mujoco_action_resampled_v2_walker2d,custom_mujoco_action_resampled_v2_ant,custom_mujoco_action_resampled_v2_humanoid}"
...
python scripts/build_custom_mujoco.py \
  --generation-mode action_resampled_v2 \
  --n-gpus "${N_GPUS}" \
...
echo
echo "== action-resampled v2 suite aggregate =="
python -m eval.summary "${RUNS_ROOT}" --out "${SUITE_ROOT}/aggregate.csv" --md "${SUITE_ROOT}/aggregate.md"
echo "Action-resampled v2 suite complete. Results in ${SUITE_ROOT}/"
