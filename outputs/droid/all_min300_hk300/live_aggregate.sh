...
set -euo pipefail
cd /mnt/data/INR
OUT=/mnt/data/INR/outputs/droid_lowdim_full_shards707of2048_p345_min300_hk300_suite_20260503
while true; do
  TS=$(date -Is)
  N=$(find "$OUT" -name summary.json | wc -l)
  if [ "$N" -gt 0 ]; then
    python -m eval.summary "$OUT" --out "$OUT/aggregate.csv.tmp" --md "$OUT/aggregate.md.tmp" >/tmp/droid_min300_aggregate.log 2>&1 || true
